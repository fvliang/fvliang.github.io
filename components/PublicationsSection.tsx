...
function GitHubButton({
  ...
  const repo = match?.[1];
  const stars =
    (repo && (starsByRepo as Record<string, number>)[repo]) ?? fallbackStars;
  ...
  return (
    <a
      ...
      className="flex-shrink-0 inline-flex items-center gap-1.5 text-xs px-3 py-1 bg-gray-50 border border-gray-200 text-gray-600 rounded-md hover:bg-gray-100 hover:shadow-md transition-all duration-300"
    >
...
const publications: Publication[] = [
  {
    ...
    links: {
      pdf: "https://arxiv.org/pdf/2607.16673",
    },
  },
  {
        ...
        <div className="space-y-6">
          ...
          {[...publications]
            ...
            <motion.div
              ...
              viewport={{ once: true, margin: "-100px" }}
            >
              <TiltCard className="px-5 py-3 bg-white/80 backdrop-blur-sm border border-gray-200 rounded-xl shadow-sm">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {pub.links.pdf && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex-shrink-0 text-xs px-3 py-1 bg-sky-50 border border-sky-200 text-sky-600 rounded-md hover:bg-sky-100 hover:shadow-md transition-all duration-300"
                    >
                      PDF
                    </a>
                  )}
                  {pub.links.github && (
                    <GitHubButton
                      github={pub.links.github}
                      fallbackStars={pub.stars ?? 0}
                    />
                  )}
                </div>
                <Authors authors={pub.authors} />
                <p className="text-sm text-sky-600">{pub.venue}</p>
              </TiltCard>
            </motion.div>
          ))}
        </div>
